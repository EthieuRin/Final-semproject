...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($password !== $confirm_password) {
        ...
        $stmt = $conn->prepare("INSERT INTO users (full_name, email, password, role) VALUES (?, ?, ?, 'host')");
        ...
        if ($stmt->execute()) {
            ...
            try {
                ...
                $mail->isHTML(true);
                $mail->Subject = 'Welcome to Event Zilla (Host)';
                $mail->Body    = '<h2>Welcome, ' . htmlspecialchars($full_name) . '!</h2>
                                  <p>Thank you for registering as an Event Host at Event Zilla. Start creating amazing events now!</p>';
...
<title>Host Registration | Event Zilla</title>
...
<div class="logo">
  ...
  <h1>EventZilla</h1>
    ...
    <h2>Register as an Event Host</h2>
